...
public interface MassBookingRepository extends JpaRepository<MassBooking, Long>, JpaSpecificationExecutor<MassBooking> {
	...
	// @Query(value = "SELECT nextval('mass_booking_no')", nativeQuery = true)
	@Query(value = "SELECT next_val from trn_mass_booking_no", nativeQuery = true)
	...
	@Query(value = "SELECT * from trn_mass_booking mb where mb.mass_time_id=?1 and mb.massBookingNo is 'CORETEAM' and mb.booked is true ORDER BY mb.seating_no", nativeQuery = true)
